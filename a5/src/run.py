...
if args.function == 'pretrain':
    ...
    fine_data = dataset.NameDataset(
        pretrain_dataset,
        open(args.finetune_corpus_path, encoding='utf-8').read()
    )

    # config param of model
    tconf = trainer.TrainerConfig(
        max_epochs=max_epochs,
        batch_size=64,  # 本地显存小，256 会 OOM；GCP/Colab 正式跑再改回 256
        learning_rate=args.finetune_lr,
        lr_decay=True,
        warmup_tokens=512 * 20,
        final_tokens=200 * len(pretrain_dataset) * block_size,
        num_workers=4,  # use GPU = 0
        writer=writer,
        ckpt_path=args.writing_params_path,
    )
    trainer.Trainer(model, fine_data, None, tconf).train()
