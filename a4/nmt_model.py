...
class NMT(nn.Module):
	...
	def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
		...
		packed_x  = pack_padded_sequence(X, source_lengths) # take off padding
		enc_hiddens, (last_hidden, last_cell) = self.encoder(packed_x) 
		enc_hiddens, _ = pad_packed_sequence(enc_hiddens) # (src, b, 2h)
		enc_hiddens = enc_hiddens.permute(1, 0, 2) # (b, src, 2h)
	...
	def step(self, Ybar_t: torch.Tensor,
		...
		dec_state = self.decoder(Ybar_t, dec_state) # decoder LSTM cell

		dec_hidden, dec_cell = dec_state # split into hidden and cell

		e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(2)).squeeze(2) # (b, src_len, 1)

		# Set e_t to -inf where enc_masks has 1
		if enc_masks is not None:
			e_t.data.masked_fill_(enc_masks.bool(), -float('inf'))

		alpha_t = F.softmax(e_t, dim=1) # (b, src_len) -> weights
		a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1) # context (b, 2h)
