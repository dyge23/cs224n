...
Options:
    ...
    --resume                                resume training from the --save-to checkpoint
...
def save_train_state(path, train_iter, epoch, num_trial, patience, hist_valid_scores):
    with open(path, 'w') as f:
        json.dump(dict(train_iter=train_iter, epoch=epoch, num_trial=num_trial,
                       patience=patience, hist_valid_scores=hist_valid_scores), f)
...
def train(args: Dict):
    ...
    resume = args['--resume'] and os.path.exists(model_save_path)

    if resume:
        print('resume training from [%s]' % model_save_path, file=sys.stderr)
    ...
    if resume and os.path.exists(model_save_path + '.optim'):
    ...
    if resume:
        trainstate_path = model_save_path + '.trainstate.json'
        if os.path.exists(trainstate_path):
            st = load_train_state(trainstate_path)
            train_iter = st['train_iter']
            epoch = st['epoch']
            num_trial = st['num_trial']
            patience = st['patience']
            hist_valid_scores = st['hist_valid_scores']
            print('resume: train_iter=%d epoch=%d num_trial=%d patience=%d' %
                  (train_iter, epoch, num_trial, patience), file=sys.stderr)
        else:
            print('warning: no [%s] found; resuming weights with fresh counters' % trainstate_path, file=sys.stderr)
        ...
        for src_sents, tgt_sents in batch_iter(train_data, batch_size=train_batch_size, shuffle=True):
            ...
            if train_iter % valid_niter == 0:
                ...
                if is_better:
                    ...
                    # also save training-loop state so training can be resumed
                    save_train_state(model_save_path + '.trainstate.json', train_iter, epoch, num_trial, patience, hist_valid_scores)
